"""Skill-aware manager for the registered ALFWorld environment."""

import logging
from typing import Optional, List

from reskill.environments.base import ReSkillEnvManagerBase
from reskill.environments.alfworld.env_manager import AlfWorldEnvironmentManager
from reskill.environments.alfworld.prompts import (
    ALFWORLD_RESKILL_REASON_NO_HIS,
    ALFWORLD_RESKILL_REASON,
...
class ReSkillAlfWorldEnvManager(ReSkillEnvManagerBase, AlfWorldEnvironmentManager):
    ...
    def set_skill_components(self, skill_loader: SkillLoader,
        ...
        logger.info(f"[ReSkillAlfWorldEnvManager] skill components set: "
                    f"memory={skill_loader}, version_ab_tracker={version_ab_tracker}")
    ...
    def build_text_obs(self, text_obs: List[str],
        ...
        tpl_no_his = ALFWORLD_RESKILL_REASON_NO_HIS
        tpl_with_his = ALFWORLD_RESKILL_REASON
        ...
        for i in range(len(text_obs)):
            ...
            if (len(obs) > self.max_obs_chars and not init
                ...
                obs = self._trim_alfworld_observation(
                    obs, text_obs[i], skills_section, i,
                    memory_contexts[i], valid_lens[i],
                    reformatted_admissible_actions, tpl_with_his, tpl_no_his)

            postprocess_text_obs.append(obs)

        return postprocess_text_obs

    def _trim_alfworld_observation(self, obs, raw_obs, skills_section, slot_idx,
                                   memory_context, valid_len,
                                   reformatted_admissible_actions, template,
                                   tpl_no_his=None):
        ...
        if len(obs) > self.max_obs_chars and tpl_no_his is not None:
            logger.warning(
                f"[ReSkillAlfWorldEnvManager] Prompt still {len(obs)} chars "
