#!/bin/bash
#SBATCH --job-name=reskill-alfworld
#SBATCH --nodes=2
...
# --- Training ---
export WANDB_NAME="reskill_alfworld_$(date +%m%d)"

cd "$REPO_DIR"
srun --nodes=1 --ntasks=1 -w "$head_node" \
    python scripts/train.py \
    --config-name alfworld \
    +ray_init.address="$head_node_ip:$port" \
    data.train_files="$REPO_DIR/data/alfworld/train.parquet" \
    data.val_files="$REPO_DIR/data/alfworld/val.parquet" \
